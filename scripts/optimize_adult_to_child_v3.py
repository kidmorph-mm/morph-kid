...
def load_target_prototype(group: str, target_height_cm: float) -> Dict[str, float]:
    ...
    target["shoulder_scale_prior"] = 0.92 if group == "core" else 0.96
    return target
...
def get_initial_params(group: str, target_h: float, source_h: float) -> np.ndarray:
    ...
    if group == "core":
        return np.array([g, 0.92, 0.95, 0.90, 0.82, 0.82, 0.82, 1.10], dtype=np.float64)
    return np.array([g, 0.96, 0.98, 0.94, 0.90, 0.90, 0.90, 1.06], dtype=np.float64)
...
DEFAULT_WEIGHT_CONFIG = {
    "feature_weights": {
        "height_canonical": 3.0,
        "shoulder_width_ratio": 6.0,
        "pelvis_width_ratio": 7.5,
...
def main():
    ...
    x0 = get_initial_params(args.group, target["height_canonical"], before_feats["height_canonical"])
    ...
    print("[INFO] after feats:", after_feats)
