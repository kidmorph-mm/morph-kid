...
def build_candidate_targets(
    ...
    for c in candidates:
        ...
        if need_stronger:
            ...
            lam = 0.35 if scale_ratio > 0.78 else 0.22
...
def build_inits(beta_init: np.ndarray, target: Dict[str, Any], before_feats: Dict[str, float]) -> List[np.ndarray]:
    ...
    scale0 = np.clip(scale_ratio, 0.78, 1.02)
    scale1 = np.clip(scale_ratio * 0.97, 0.72, 1.00)
    scale2 = np.clip(scale_ratio * 0.93, 0.68, 0.98)
...
def optimize_for_candidate(
    ...
    starts = build_inits(kid_beta_init, target, before_feats)
    bounds = [(-4.0, 4.0)] * beta_dim + [(-0.28, 0.06)]
...
def process_one(
    ...
    candidates = build_candidate_targets(before_feats, all_protos, label_row=label_row, topk=TOPK_CANDIDATES)
...
def run_batch(
    ...
    random.Random(seed).shuffle(adult_paths)
