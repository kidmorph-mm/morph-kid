...
DEFAULT_WEIGHT_CONFIG = {
    "feature_weights": {
        "height_canonical": 4.2,
        "shoulder_width_ratio": 8.0,
        "pelvis_width_ratio": 10.0,
        "torso_height_ratio": 4.8,
        "arm_length_ratio": 7.0,
        "thigh_ratio": 5.2,
        "shank_ratio": 5.2,
        "leg_length_ratio": 6.0,
    },
    "prior_weights": {
        "shoulder_scale_prior": 0.35,
        "head_scale_prior": 1.6,
    },
    "reg_weights": {
        "global_scale": 1.1,
        "shoulder_scale": 0.10,
...
def get_initial_params(group: str, target_h: float, source_h: float) -> np.ndarray:
    g = target_h / max(source_h, 1e-8)
    if group == "core":
        return np.array([g, 0.94, 0.96, 0.96, 0.84, 0.84, 0.78, 1.08], dtype=np.float64)
    return np.array([g, 0.98, 0.99, 0.98, 0.92, 0.92, 0.84, 1.05], dtype=np.float64)
...
def load_balanced_target(adult_pkl: str | Path, label_csv: Path) -> Dict[str, Any]:
    ...
    return {
        ...
        "head_scale_prior": 1.08 if str(row["assigned_group"]) == "core" else 1.05,
        "shoulder_scale_prior": 0.98 if str(row["assigned_group"]) == "core" else 1.00,
        "source_assignment_score": float(row["assignment_score"]),
        "source_height_bin": str(row["assigned_height_bin"]),
    }


def objective_factory(can_verts, can_joints, target, weight_cfg):
    ...
    def obj(x: np.ndarray) -> float:
        ...
        # arm이 target보다 과도하게 짧아지는 것 억제
        arm_target = target["arm_length_ratio"]
        arm_now = feats["arm_length_ratio"]
        if arm_now < arm_target:
            loss += 4.0 * (arm_target - arm_now) ** 2

        # shoulder가 target보다 과도하게 작아지는 것 억제
        sh_target = target["shoulder_width_ratio"]
        sh_now = feats["shoulder_width_ratio"]
        if sh_now < sh_target:
            loss += 4.5 * (sh_target - sh_now) ** 2
...
def main():
    ...
    print("[INFO] assigned group:", group)
